...
namespace ProcessSimulation
{
    public partial class DBMainForm : Form
    {
        ...
        private void button1_Click_1(object sender, EventArgs e)
        {
            ...
            DBMainForm mainForm = new DBMainForm();
        ...
        private void ApplyResourcebutton_Click(object sender, EventArgs e)
        {
            ...
            if (checkApplyResNeed(pro))//pro.Request[i]<=pro.Need[i],进程申请资源数小于其需求数
            {
                ...
                {
                    DeadLockBimian bankfrm = new DeadLockBimian(this);//创建新的副窗体
                    copyto(this.allProgress, DeadLockBimian.allProgress);//创建进程副本
                    DeadLockBimian.progressNumber = this.progressNumber;//向副窗体传递进程数目
                    this.Available.CopyTo(DeadLockBimian.Available, 0);//向父窗体传递目前系统可利用资源数目
